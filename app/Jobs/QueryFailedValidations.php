...
class QueryFailedValidations implements ShouldQueue
{
            ...
            if (! empty($token)) {
                ...
                if (! empty($validationReport)) {
                    ...
                    if ($validationReport['data']['hasMore'] === false) {
                        $paymentFile->failed_validation = "completed";
                        $paymentFile->save();
                    }
                    ...
                    \Log::info("Invalid Validation Status Captured Successfully");

                } else {

                    \Log::error("Querying Invalid Validation Report Failed");
    ...
    public function queryValidationReport($fileId, $token)
    {
        ...
        ])->post(env('RESIDENT_BASE_URL') . '/retrieveinvalidvalidation', $data);
