...
<head>
    ...
    <title>Deduction Summary Report Per Pay Station</title>
    <!-- Bootstrap CSS -->
    ...
    <div class="container">
        ...
        <div class="ms-3 row mb-5 mt-5 me-2">
            ...
            <div class="col-6" style="color:black">
                ...
                <span style="font-size:16px; display:block;">
                    @if ($period == 'All')
                        All Time Deduction Summary Report Per Pay Station
                    @else
                        Deduction Summary Report Per Pay Station For {{ $period }}
                    @endif
                </span>
        ...
        @if ($branch == 'All')
            ...
            @foreach ($branches as $branch)
                <h6>Pay Station: {{ $branch->branch }}</h6>
                <table class="table table-bordered table-striped mb-5"
                    ...
                    <tr>
                        ...
                        <th class="text-end">NLC Contribution (=N=)</th>
                    ...
                    @foreach ($deductions as $data)
                        @php
                            $deductionTotal = $data->deductions($workgroup->id, $period, $data->sql_label, $branch->id);
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                @php
                                    $headcount =
                                        $headcount +
                                        $data->headCount($workgroup->id, $period, $data->sql_label, $branch->id);
                                @endphp
                                {{ number_format($data->headCount($workgroup->id, $period, $data->sql_label, $branch->id), 0) }}
                            </td>
                            <td class="text-end">{{ number_format($deductionTotal, 2) }}</td>
                            <td class="text-end">{{ number_format($data->nlc_contribution, 2) }}</td>
                            <td class="text-end">{{ number_format($deductionTotal, 2) }}</td>
                            <td>{{ $data->bank }}</td>
                    ...
                    <tr style="background:#F8E4AC">
                        <th colspan="2" class="text-end" style="color:black">TOTALS</th>
            ...
            @endforeach
        @else
            <h6>Pay Station: {{ $branchName->branch }}</h6>
            <table class="table table-bordered table-striped mb-5"
                style="font-size:10px; font-family:Arial, Helvetica, sans-serif">
                <tr class="cate table-light">
                <tr>
                ...
                @foreach ($deductions as $data)
                    @php
                        $deductionTotal = $data->deductions($workgroup->id, $period, $data->sql_label, $branchName->id);
                        $grandTotal = $grandTotal + $deductionTotal;
                    @endphp
                    <tr>
                        <td>{{ $loop->index + 1 }}</td>
                        <td>{{ $data->deduction_name }}</td>
                        <td class="text-end">
                            {{ number_format($data->headCount($workgroup->id, $period, $data->sql_label, $branchName->id), 0) }}
                        </td>
                ...
                @endforeach
                <tr style="background:#F8E4AC">
                    <th colspan="3" class="text-end" style="color:black">GRAND TOTAL</th>
                    <th style="color:black">{{ number_format($grandTotal, 2) }}</th>
                    <th></th>
                    <th style="color:black">{{ number_format($grandTotal, 2) }}</th>
                    <th></th>
                    <th></th>
